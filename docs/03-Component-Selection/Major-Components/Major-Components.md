...
> **Rationale:** The SparkFun 3.2" display is the sweet spot - bigger screen than the Adafruit 2.4" but same price at $20. The extra size matters when you're cramming rover status, motor data, sensor readings and menus on one screen. Color coding (red for errors, green for good, yellow for warnings) makes it way faster to see what's up compared to black and white. The ILI9341 controller has tons of ESP32 libraries and examples so getting it running is easy. Touchscreen means we can do a better UI than just mashing buttons. Yeah SPI takes more pins than I2C but the ESP32 has GPIO to spare. The Adafruit 2.4" version costs $10 more and is smaller so that's out. The monochrome OLED is same price but way too small and can't do colors which you really want for a safety board showing status.
