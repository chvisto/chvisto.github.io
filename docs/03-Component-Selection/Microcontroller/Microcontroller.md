...
> **Rationale:** The ESP32-S3 is perfect for what we're doing right now but it gives us room to grow. The dual UART makes the daisy-chain network easy to handle without any problems. Hardware SPI means the TFT LCD updates fast and smooth without bogging down the CPU. It uses more power than a PIC but we got a 3A regulator so that's not an issue.
